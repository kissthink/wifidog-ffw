...
main_loop(void)
{
	...
	if (!config->gw_address) {
		for (p = config->gw_interface ; p != NULL ; p = p->next)
		 {
			debug(LOG_DEBUG, "Finding IP address of %s", p->interface);
			if ((config->gw_address = get_iface_ip(p->interface)) == NULL) {
				debug(LOG_ERR, "Could not get IP address information of %s, exiting...", p->interface);
			exit(1);
			}
			debug(LOG_DEBUG, "%s = %s", p->interface, config->gw_address);
		}
	}
	...
	if (!config->gw_id) {
		for (p = config->gw_interface ; p != NULL ; p = p->next)
			{
    				debug(LOG_DEBUG, "Finding MAC address of %s", p->interface);
    				if ((config->gw_id = get_iface_mac(p->interface)) == NULL) {
				debug(LOG_ERR, "Could not get MAC address information of %s, exiting...", p->interface);
				exit(1);
				}
				debug(LOG_DEBUG, "%s = %s", p->interface, config->gw_id);
			}
	}
