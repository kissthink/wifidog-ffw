...
/*
 * $Id: util.c 1241 2007-06-24 04:13:13Z benoitg $
 */
/**
  @file util.c
...
char * get_status_text(int showtrusted) {
	...
	snprintf((buffer + len), (sizeof(buffer) - len), "Node Name: %s\n", config_get_config()->NodeName);
	len = strlen(buffer);
	...
	for (gwi = config_get_config()->gw_interface ; gwi != NULL ; gwi = gwi->next)
	  {
		snprintf((buffer + len), (sizeof(buffer) - len), "Interface : %s\n", gwi->interface);
		len = strlen(buffer);
	  }
	...
	if(showtrusted)
	  {
	    if((proxy_port = config_get_config()->proxy_port) != 0)
	      {
	        snprintf((buffer + len), (sizeof(buffer) - len), "ProxyPort : %u\n", proxy_port);
	        len = strlen(buffer);
	      }
